...
def select_dir():
    dir_path = dir_input.get() #retrieves input from widget
    print(HOST, PORT)
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM, 0) as s:
            with client_context.wrap_socket(s, server_hostname=HOST) as ssl_socket:
                ...
                flag = ssl_socket.recv(1024).decode()
                if flag == 'True': #indicates directory is present on the server side
                    ...
                    with open(filename, 'rb') as f:
                        ...
                        while data:
                            ...
                            if i % step_size == 0:
                                ...
                                hashes = '#' * int(percent * bar_length)    #no. of hashes
                    ...
                    dir_text.insert(END, ssl_socket.recv(1024).decode())#inserts the message received from the server at the end of the text widget.
                else:
                    dir_text.delete(1.0, END)
